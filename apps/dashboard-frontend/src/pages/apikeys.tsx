...
const ApiKeys = () => {
  ...
  const { data: apiKeys = [], isLoading, error } = useQuery<ApiKey[]>({
    ...
    queryFn: async () => {
      ...
      return (response.data || []).filter((key: any) => !key.isDeleted)
    },
    retry: 1,
  })
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      // Invalidate the query to refetch and filter out the deleted key
      queryClient.invalidateQueries({ queryKey: ['api-keys'] })
    },
    onError: () => {
                    ...
                    {apiKeys.map((key) => (
                      <tr key={key.id} className="border-b border-border hover:bg-muted/50">
                        ...
                        <td className="py-3 px-4">
                          <span
                            className={`text-xs px-2 py-1 rounded-full ${key.isDisabled
